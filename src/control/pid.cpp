...
float PID::update(float error) {
  ...
  bool signFlip = m_lastError != NAN && (std::copysign(1.0, error) !=
                                         std::copysign(1.0, m_lastError));

  if (signFlip)
    m_isSettling = true;

  m_integral += error * dt;
  if (error > m_iMax || signFlip) {
    m_integral = 0;
  }

  float deltaError = (m_lastError == NAN) ? 0 : (error - m_lastError) / dt;
  m_lastError = error;
